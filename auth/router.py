...
def login(request: schema.Login,
          database: Session = Depends(get_db)) -> schema.UserLogin:    
...
async def get_profile(database: Session = Depends(get_db), current_user: schema.User = Depends(get_current_user)):
